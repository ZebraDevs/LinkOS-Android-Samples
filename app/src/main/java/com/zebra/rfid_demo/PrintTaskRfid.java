...
public class PrintTaskRfid extends AsyncTask<Void, Void, Void> {
    ...
    private String ZPL_PRICE_LABEL_DATA = null;
    ...
    public PrintTaskRfid(View v, View.OnClickListener view2, Context context, String [] lastRecord, int type_label, final LinearLayout layoutScan) {
    ...
    public HashMap<Integer, String> mergeVariablesRetailLabel() {
        ...
        if (discoveredPrinter != null) {
          Connection connection = new BluetoothConnection(discoveredPrinter.address);
            try {
                ...
                if(myLabel_type==0) {
                    configLabel = ZPL_RFID_LABEL.getBytes();
                }else if(myLabel_type==1){
                    configLabel = ZPL_PRICE_LABEL.getBytes();
                   // variableData = ZPL_PRICE_LABEL_DATA.getBytes();
                }else if(myLabel_type==2){
                    configLabel=ZPL_RFID_LABEL.getBytes();
                }
                if (printerStatus.isReadyToPrint) {
                    Snackbar.make(myMainView, R.string.print_successful, Snackbar.LENGTH_LONG).show(); // Don’t forget to show!
                    try {
                        connection.write(configLabel);
                     //   connection.write(variableData);
                        if(myLabel_type==1) {
                            zebraPrinter.printStoredFormat("E:LabelSt.ZPL", mergeVariablesRetailLabel());
                        }
                        connection.close();
                        ...
                        try {
                            Snackbar.make(myMainView, R.string.Language_is_LINE_PRINT, Snackbar.LENGTH_LONG).show(); // Don’t forget to show!
                            SGD.SET("device.languages", "zpl", connection);
                        } catch (ConnectionException ex) {
                            Snackbar.make(myMainView, R.string.connection_error, Snackbar.LENGTH_LONG).show();
                        }
                    }

                } else if (printerStatus.isPaused) {
                    Snackbar.make(myMainView, R.string.print_failed+" "+R.string.printer_paused, Snackbar.LENGTH_LONG).show();
                } else if (printerStatus.isHeadOpen) {
                    Snackbar.make(myMainView, R.string.print_failed+" "+R.string.head_open, Snackbar.LENGTH_LONG).show();
                } else if (printerStatus.isPaperOut) {
                    Snackbar.make(myMainView, R.string.print_failed+" "+R.string.paper_out, Snackbar.LENGTH_LONG).show();
                } else {
                    Snackbar.make(myMainView, R.string.print_failed+" "+R.string.cannot_print, Snackbar.LENGTH_LONG).show();
                }
