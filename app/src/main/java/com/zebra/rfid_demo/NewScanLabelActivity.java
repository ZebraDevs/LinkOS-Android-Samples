    ...
    private void setFieldData(String [] splitBarcode) {
        final LinearLayout fieldsLayout = (LinearLayout) findViewById(R.id.fieldsLayout);
            ...
            try {
                ...
                Log.d(SCANNED_INTENT_TAG,"Scan detected _Print: " +  BarcodeData_db[0]+"," +BarcodeData_db[1]+","+BarcodeData_db[2]+","+BarcodeData_db[3]+","+ BarcodeData_db[4]+"," +BarcodeData_db[5]+","+BarcodeData_db[6]+","+BarcodeData_db[7]+",");

                if(!BarcodeData_db[0].isEmpty()) {
                    addNewEntry(BarcodeData_db);
                    new PrintTaskRfid(context, BarcodeData_db, PRINT_RFID_LABEL, fieldsLayout).execute((Void[]) null);
                }
